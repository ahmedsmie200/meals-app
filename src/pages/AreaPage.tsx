...
export default function AreaPage({ currentPage, onNavigate, onMealClick }) {
  ...
  const fetchAreas = async () => {
    ...
    } catch (e) { console.error(e); } finally { setLoadingAreas(false); }
  };
  ...
  const fetchMealCounts = async (areaList) => {
    ...
    await Promise.all(areaList.map(async (a) => {
      ...
      } catch { counts[a.strArea] = 0; }
    }));
    setMealCounts(counts);
  };

  const fetchMealsByArea = async (area) => {
    setSelectedArea(area); setLoadingMeals(true); setMeals([]);
    try {
      const res = await fetch(`${API_BASE}/filter.php?a=${encodeURIComponent(area)}`);
      const data = await res.json();
      setMeals(data.meals || []);
    } catch (e) { console.error(e); } finally { setLoadingMeals(false); }
  };

  const filteredAreas = areas.filter((a) => a.strArea.toLowerCase().includes(search.toLowerCase()));

  return (
    <div className="min-h-screen bg-[#FDF8F3] p-4 sm:p-8">
      <div className="mb-6">
        <h2 className="text-2xl sm:text-3xl font-bold text-[#3E1F00] mb-1 flex items-center gap-2" style={{ fontFamily: "Pacifico, cursive" }}>
          <Globe className="w-7 h-7 text-amber-600" /> World Cuisines
        </h2>
        <p className="text-amber-700 text-sm">Explore recipes from {areas.length} cuisines around the globe</p>
      </div>

      <div className="relative mb-6 max-w-md">
        <input type="text" placeholder="Search cuisine..." value={search} onChange={(e) => setSearch(e.target.value)}
          ...
          {!selectedArea && (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-8">
              {filteredAreas.map((area) => (
                <button key={area.strArea} onClick={() => fetchMealsByArea(area.strArea)}
                  ...
                  <div className="text-4xl mb-2 group-hover:scale-110 transition-transform duration-200">{areaFlags[area.strArea] || "🌍"}</div>
                  <p className="text-[#3E1F00] font-semibold text-sm">{area.strArea}</p>
                  {mealCounts[area.strArea] !== undefined && <p className="text-amber-500 text-xs mt-1">{mealCounts[area.strArea]} meals</p>}
                </button>
              ))}
              {filteredAreas.length === 0 && <p className="col-span-full text-amber-600 text-center py-10">No cuisines found</p>}
            </div>
          )}
          ...
          {selectedArea && (
            <div>
              <div className="flex items-center gap-3 mb-6">
                <button onClick={() => { setSelectedArea(null); setMeals([]); }}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    {!loadingMeals && <p className="text-amber-500 text-xs">{meals.length} recipes</p>}
                  </div>
                </div>
              </div>

              {loadingMeals ? (
                <div className="flex justify-center py-16"><div className="w-10 h-10 border-4 border-amber-600 border-t-transparent rounded-full animate-spin"></div></div>
              ) : meals.length === 0 ? (
                <p className="text-amber-600 text-center py-10">No meals found</p>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                  {meals.map((meal) => (
                    <div key={meal.idMeal} onClick={() => onMealClick && onMealClick(meal.idMeal)}
                      ...
                      <div className="overflow-hidden">
                        <img src={meal.strMealThumb} alt={meal.strMeal} className="w-full h-32 object-cover group-hover:scale-105 transition-transform duration-300" />
                      </div>
                      <div className="p-3">
                        <p className="text-[#3E1F00] font-semibold text-xs leading-tight line-clamp-2">{meal.strMeal}</p>
                      </div>
                    </div>
                  ))}
                </div>
